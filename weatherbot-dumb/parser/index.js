...
let currentTemperature = (location, response) => {
    if (response.icon) {
        ...
        return `Right now, temperature in ${location.toUpperCase().green} is ${getFeel(Number(temperature))} at ${temperature.toString().red.bold} degrees, feels like ${apparentTemperature.toString().red.bold} degrees.`;
    } else {
        return `Sorry, I can't get the weather for ${location.toUpperCase().red.bold}.`;
    }
};
